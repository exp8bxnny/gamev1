...
// twin sprite transparency
if(battle_state == "twin_attack"){
	twin_alpha = 0;
} else {
	twin_alpha = 1;
}
...
else if (battle_state == "result")
{
    ...
    if (keyboard_check_pressed(ord("Z")) && input_delay <= 0)
    {
        ...
        else
        {
			...
			if (enemy_hp > 0)
			{
			    // Begin the twin's turn
			    battle_state = "twin_attack";
	        ...
	        else
	        {
				...
				dialogue_text = "* You defeated the twin!";
	            visible_characters = 0;
	            dialogue_finished = false;
...
else if (battle_state == "twin_attack")
{
	
    var seconds = delta_time / 1000000;
    twin_turn_timer -= seconds;
    ...
    {
        array_push(twin_attack_x, random(1));
        array_push(twin_attack_y, 0);
    ...
    for (var i = array_length(twin_attack_y) - 1; i >= 0; i--)
    {
        twin_attack_y[i] += attack_fall_speed * seconds;
        ...
        )
        {
            ...
            array_delete(twin_attack_x, i, 1);
            array_delete(twin_attack_y, i, 1);
    ...
    // End the twin's turn
    if (twin_turn_timer <= 0 && battle_state == "twin_attack")
    {
...
else if (battle_state == "victory")
{
    ...
    if (keyboard_check_pressed(ord("Z")) && input_delay <= 0)
    {
        ...
        {
            // Remember that the twin was defeated
